...
contract InvariantsTest is Assertions, Logging, BaseInvariantTest, BaseMultiCollateralTest {
    ...
    function setUp() public override {
        ...
        if (n > 0) {
            p[0] = TestDeployer.TroveManagerParams(1.5 ether, 1.1 ether, 0.1 ether, 1.1 ether, 100_000_000 ether, 0.05 ether, 0.1 ether, false, address(0));
        }
        if (n > 1) {
            p[1] = TestDeployer.TroveManagerParams(1.6 ether, 1.2 ether, 0.1 ether, 1.2 ether, 100_000_000 ether, 0.05 ether, 0.2 ether, false, address(0));
        }
        if (n > 2) {
            p[2] = TestDeployer.TroveManagerParams(1.6 ether, 1.2 ether, 0.1 ether, 1.2 ether, 100_000_000 ether, 0.05 ether, 0.2 ether, false, address(0));
        }
        if (n > 3) {
