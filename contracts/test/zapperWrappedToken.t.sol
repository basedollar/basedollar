...
contract ZapperWrappedTokenTest is DevTestSetup {
    ...
    function setUp() public override {
        ...
        troveManagerParams[0] = TestDeployer.TroveManagerParams(150e16, 110e16, 10e16, 110e16, 10_000_000e18, 5e16, 10e16, false, address(0));
        troveManagerParams[1] = TestDeployer.TroveManagerParams(150e16, 110e16, 10e16, 110e16, 10_000_000e18, 5e16, 10e16, false, address(0));

        TestDeployer deployer = new TestDeployer();
        TestDeployer.LiquityContractsDev[] memory contractsArray;
        TestDeployer.Zappers[] memory zappersArray;
        TestDeployer.DeployAndConnectContractsResults memory deployedContracts =
        ...
        // A to F: underlying for troves + ETH for gas compensation (mirrors zapperWETH.t.sol)
        for (uint256 i = 0; i < 6; i++) {
            deal(address(underlyingToken), accountsList[i], initialCollateralAmount);
            deal(accountsList[i], 10_000 ether);
    ...
    function testCanNotOpenTroveWithBatchManagerAndInterest() external {
        ...
        wrappedTokenZapper.openTroveWithRawETH{value: ETH_GAS_COMPENSATION}(params);
        vm.stopPrank();
    }
