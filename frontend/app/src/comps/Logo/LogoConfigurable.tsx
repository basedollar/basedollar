import { useWhiteLabelHeader } from "@/src/hooks/useWhiteLabel";
import Image from "next/image";

// Original hardcoded logo as fallback
function DefaultLogo({ size = 32 }: { size?: number }) {
  return (
    <svg
      fill="none"
      height={size}
      width={size}
      viewBox="0 0 32 32"
    >
      <path
        d="M31.097 10.687c-.295-.013-.593-.02-.891-.02-10.526 0-19.109 8.316-19.54 18.735V.91C4.453 3.107 0 9.034 0 16 0 7.163 7.163 0 16 0c6.974 0 12.906 4.462 15.097 10.687ZM10.667 31.09a15.456 15.456 0 0 0 .002 0l-.002-.045v.045Z"
        fill="#1C1D4F"
      />
      <path
        d="M16 32c8.837 0 16-7.163 16-16 0-1.862-.318-3.65-.903-5.313-.295-.013-.593-.02-.891-.02-10.8 0-19.556 8.756-19.556 19.556 0 .291.006.58.019.867 1.667.59 3.462.91 5.331.91Z"
        fill="#F5D93A"
      />
      <path
        d="M10.667 31.09C4.452 28.893 0 22.967 0 16h10.667v15.09Z"
        fill="#405AE5"
      />
      <path
        d="M0 16h10.667V.91C4.452 3.107 0 9.034 0 16Z"
        fill="#FB7C59"
      />
    </svg>
  );
}

export function LogoConfigurable({
  size = 32,
  variant = "main",
}: {
  size?: number;
  variant?: "main" | "icon";
}) {
  const headerConfig = useWhiteLabelHeader();
  
  // Determine which logo to use based on variant
  const logoPath = variant === "icon" ? headerConfig.logo.icon : headerConfig.logo.main;
  
  // If no custom logo configured, use default
  if (!logoPath || logoPath.includes("/brand/")) {
    return <DefaultLogo size={size} />;
  }
  
  // Use custom logo from config
  return (
    <div style={{ width: size, height: size, position: "relative" }}>
      <Image
        src={logoPath}
        alt={headerConfig.logo.alt}
        fill
        style={{ objectFit: "contain" }}
      />
    </div>
  );
}