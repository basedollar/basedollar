...
export const earnUpdate: FlowDeclaration<EarnUpdateRequest> = {
  ...
  steps: {
    ...
    // Unwrap wraped collateral reward
    unwrapCollReward: {
      name: (ctx) => {
      ...
      async commit(ctx) {
        ...
        return ctx.writeContract({
          ...
          args: [
            ...
            ctx.request.earnPosition.rewards.coll[0] > wrappedBalance
              ? wrappedBalance
  ...
  async getSteps(ctx) {
    const { earnPosition, prevEarnPosition, claimRewards } = ctx.request;
    const branch = getBranch(earnPosition.branchId);
    ...
    if (
      claimRewards
      && dn.gt(earnPosition.rewards.coll, 0)
