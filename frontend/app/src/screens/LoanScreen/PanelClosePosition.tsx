import type { PositionLoanCommitted, TokenSymbol } from "@/src/types";
...
import { addressesEqual, Dropdown, TokenIcon, CollateralIcon, VFlex } from "@liquity2/uikit";
...
export function PanelClosePosition({
  ...
  const repayToken = repayWithCollateral ? collateral.symbol : WHITE_LABEL_CONFIG.tokens.mainToken.symbol as TokenSymbol;
  ...
  // either in {WHITE_LABEL_CONFIG.tokens.mainToken.symbol} or in collateral
            ...
            field={
              ...
              >
                ...
                >
                  <Amount
                    value={amountToRepay}
                    title={{ suffix: ` ${repayToken === WHITE_LABEL_CONFIG.tokens.mainToken.symbol ? WHITE_LABEL_CONFIG.tokens.mainToken.symbol : collateral.name}` }}
                    fallback="−"
                  />
                </div>
                <Dropdown
                  buttonDisplay={() => ({
                    icon: <TokenIcon symbol={repayToken === WHITE_LABEL_CONFIG.tokens.mainToken.symbol ? WHITE_LABEL_CONFIG.tokens.mainToken.symbol : collateral.symbol} />,
                    label: (
                      <>
                        {repayToken === WHITE_LABEL_CONFIG.tokens.mainToken.symbol ? WHITE_LABEL_CONFIG.tokens.mainToken.symbol : collateral.name}
                  ...
                  items={[
                    {
                      ...
                      label: <div className={css({ whiteSpace: "nowrap" })}>{WHITE_LABEL_CONFIG.tokens.mainToken.symbol} (account)</div>,
            ...
            footer={{
              start: <Field.FooterInfo label={fmtnum(amountToRepayUsd, { preset: "2z", prefix: "$" }) || "−"} />,
                ...
                <Field.FooterInfoPriceImpact
                  ...
                  outputTokenName={WHITE_LABEL_CONFIG.tokens.mainToken.symbol}
      ...
      <FlowButton
        ...
        disabledReason={
          !isOwner
            ...
            : !claimOnly && repayToken === WHITE_LABEL_CONFIG.tokens.mainToken.symbol
                && amountToRepay
